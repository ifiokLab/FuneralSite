...
function addFormsetRow() {
  ...
  const formsetCount = parseInt(formset.querySelector('#id_photoalbum_set-TOTAL_FORMS').value);
  ...
  newFormsetRowInputs.forEach((input) => {
    ...
    if(input.getAttribute('name').endsWith('title')){
      input.value = '';
    } 
 ...
 if (formsetCount > 1) {
    const formsetId = formsetRow.lastElementChild.getAttribute('data-formset-id');
      if(formsetId === 'None'){
        ...
        const url = `/delete-photo/${formsetId}/`; // Change this to the URL for deleting the formset
        fetch(url, {
          method: 'DELETE',
          headers: {
